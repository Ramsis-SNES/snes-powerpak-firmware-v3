...
; ***************************** Warm boot ******************************
	...
	jmp ROMBrowserLoop			; return to ROM browser
...
; This routine loads the root directory into the SDRAM buffer, and
; prints the content to the screen. Expects extNum/extMatchX variables
...
__InitFileBrowserOtherDir:			; jsr here with another directory's source cluster set (e.g. SAVES, THEMES)
	lda #%00000011				; use SDRAM buffer, skip hidden files
	sta CLDConfigFlags

	jsr CardLoadDir				; reminder: CLDConfigFlags is reset upon RTS

	rep #A_8BIT				; A = 16 bit

	lda filesInDir				; check if root dir contains relevant files beside the "POWERPAK" folder
	beq __IFBRootDirEmpty
...
.ACCU 16
	...
	pla					; clean up the stack as there's no rts from "jsr InitFileBrowser" in case of an error
